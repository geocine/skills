param(
    ...
    [string]$Folder,
    [string]$Out = "cursor-folder-history.json",
    [string]$Contains,
    [Nullable[int]]$Limit,
    [string]$CursorRoot,
    [string]$GitBashPath,
...
function Resolve-CondaShPath {
    ...
    throw "Unable to find conda.sh. Set -CondaShPath explicitly."
}

$gitBashExe = Resolve-GitBashPath -Override $GitBashPath
$condaShWin = Resolve-CondaShPath -Override $CondaShPath

$traversalScriptWin = Join-Path $PSScriptRoot "traverse_cursor_history.py"
...
$outArg = $Out -replace "\\", "/"
$cursorRootArg = $null
if (Test-NonEmpty $CursorRoot) {
    $cursorRootArg = $CursorRoot -replace "\\", "/"
}
...
if (Test-NonEmpty $cursorRootArg) {
    $pythonParts += "--cursor-root $(Bash-Quote $cursorRootArg)"
}
