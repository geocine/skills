param(
    ...
    [string]$Folder,
    [string]$Out = "grok-folder-history.json",
    [string]$Contains,
    [Nullable[int]]$Limit,
    [switch]$Brief,
    [string]$GrokRoot,
...
$traversalScriptWin = Join-Path $PSScriptRoot "traverse_grok_history.py"
...
$outArg = $Out -replace "\\", "/"
$grokRootArg = $null
if (Test-NonEmpty $GrokRoot) {
    $grokRootArg = $GrokRoot -replace "\\", "/"
}
...
if (Test-NonEmpty $grokRootArg) {
    $pythonParts += "--grok-root $(Bash-Quote $grokRootArg)"
}
