...
open_verification_page() {
  ...
  elif is_windows_platform && command -v cmd.exe >/dev/null 2>&1; then
...
update_launcher() (
  # The current upstream launcher has no Windows platform branch. Keep this
  # Windows-compatible launcher in place until an upstream Windows-aware
  # launcher is published.
...
update_binary() (
  ...
  local actual_size actual_checksum old_binary suffix
  ...
  candidate="$work/muse-bin$suffix"
  ...
  target="$dir/muse-bin-$version$suffix"
  installed_version=""
  ...
  rm -f -- "$dir/muse-bin" "$dir/muse-bin.exe"
)
