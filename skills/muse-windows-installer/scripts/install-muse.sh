...
launcher_file="${MUSE_LAUNCHER_FILE:-}"

# When this installer and the patched launcher are kept together, Git Bash
# should use the local Windows-compatible launcher instead of downloading the
# current Unix-only upstream launcher.
if [[ -z "$launcher_file" ]]; then
  case "${OSTYPE:-}" in
    msys*|cygwin*|win32*)
      script_dir="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
      local_launcher="$script_dir/muse-launcher.sh"
      [[ ! -r "$local_launcher" ]] || launcher_file="$local_launcher"
      ;;
  esac
fi
launcher_tmp=""
...
_muse_install() {
  ...
  if [[ -n "$launcher_file" ]]; then
    [[ -r "$launcher_file" ]] || die "launcher file is not readable: $launcher_file"
    cp -- "$launcher_file" "$launcher_tmp"
  else
    curl \
      --fail \
      --silent \
